...
class RRTagController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func createNewTag() {
        ...
        if strlen(contentTag) > 0 {
            let newTag = Tag(isSelectedTag: false, isLocked: false, textContent: contentTag)
    ...
    func collectionView(collectionView: UICollectionView, didSelectItemAtIndexPath indexPath: NSIndexPath) {
        ...
        if indexPath.row < tags.count {
            ...
            if tags[indexPath.row].isSelectedTag == false {
                tags[indexPath.row].isSelectedTag = true
                selectedCell?.animateSelection(tags[indexPath.row].isSelectedTag)
                totalTagsSelected = 1
            }
            else {
                tags[indexPath.row].isSelectedTag = false
                selectedCell?.animateSelection(tags[indexPath.row].isSelectedTag)
                totalTagsSelected = -1
            }
        }
    ...
    class func displayTagController(#parentController: UIViewController, tagsString: [String]?,
        ...
        let tagController = RRTagController()
            ...
            if tagsString != nil {
                for currentTag in tagsString! {
                    tagController.tags.append(Tag(isSelectedTag: false, isLocked: false, textContent: currentTag))
                }
            }
